...
class PropertySuggestionController extends Controller
{
    ...
    public function autocomplete(Request $request)
    {
        ...
        return Cache::remember($cacheKey, 3600, function () use (
            $query, $type, $addressLimit, $buildingLimit, $placeLimit
        ) {
            $baseQuery = BridgeProperty::query()
                ->when($type === 'buy', fn($q) =>
                    $q->whereIn('standard_status', ['Active', 'Active Under Contract', 'Pending'])
            ...
            $addressSuggestions = (clone $baseQuery)
                ...
                ->map(function ($item) {
                    $address = trim("{$item->street_number} {$item->street_name}");
                    if ($item->unit_number) $address .= " #{$item->unit_number}";
                    return [
                        'type' => 'address',
                        'id' => $item->id,
                        'listing_key' => $item->listing_key,
                        ...
                        'price' => $item->list_price,
                        'status' => $item->standard_status,
                        'display_text' => "{$address}, {$item->city}, {$item->state_or_province}",
                        'action_url' => "/api/properties/{$item->id}"
                    ];
                });
        
            // Building Suggestions
            $buildingSuggestions = DB::table(DB::raw('(SELECT * FROM bridge_properties WHERE street_number IS NOT NULL AND street_name IS NOT NULL AND property_type NOT LIKE "%ResidentialLease%") as bp'))
                ->leftJoin('bridge_property_details as bpd', 'bp.id', '=', 'bpd.property_id')
                ->select(
                    'bp.street_number',
                    'bp.street_name',
                ...
                ->get()
                ->map(function ($item) {
                    $address = trim("{$item->street_number} " . ($item->street_dir_prefix ? "{$item->street_dir_prefix} " : '') . "{$item->street_name}");
                    $buildingName = $item->building_name ?? $address;
                    return [
                        'type' => 'building',
                        'building_name' => $buildingName,
                        'street_number' => $item->street_number,
                        ...
                        'property_sub_type' => $item->property_sub_type,
                        'unit_count' => $item->unit_count,
                        'display_text' => "{$buildingName}, {$item->city}, {$item->state_or_province} ({$item->unit_count} units)",
                        'action_url' => "/api/buildings?street_number={$item->street_number}&street_name=" . urlencode($item->street_name)
                    ];
                });
        
            // Place Suggestions (City, State, Postal)
            $cities = DB::table('bridge_properties')
                ...
                ->map(fn($item) => [
                    ...
                    'name' => $item->city,
                    'state' => $item->state_or_province,
                    'display_text' => "{$item->city}, {$item->state_or_province}",
                ]);
            ...
            $states = DB::table('bridge_properties')
                ...
                ->map(fn($item) => [
                    'type' => 'state',
                    'name' => $item->state_or_province,
                    'display_text' => $item->state_or_province,
                ]);
